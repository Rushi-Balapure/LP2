...
def key_expansion(key: bytes, nb: int = 4):
...
def add_round_key(state, key_schedule, round: int):
...
def state_from_bytes(data: bytes):
...
def inv_mix_column_optimized(col):
...
def inv_mix_columns_optimized(state):
    for r in state:
